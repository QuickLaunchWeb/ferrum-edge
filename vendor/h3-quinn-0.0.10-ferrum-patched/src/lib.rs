...
pub struct SendStream<B: Buf> {
    ...
    writing: Option<Vec<Bytes>>,
    write_chunks_fut: WriteChunksFuture,
...
type WriteChunksFuture =
    ReusableBoxFuture<'static, (quinn::SendStream, Result<(), quinn::WriteError>)>;
    ...
    B: Buf,
{
    fn new(stream: quinn::SendStream) -> SendStream<B> {
        ...
        Self {
            ...
            writing: None,
            // Should only allocate once the first time it's used.
            write_chunks_fut: ReusableBoxFuture::new(async { unreachable!() }),
    ...
    fn reset_quinn_stream(stream: &mut quinn::SendStream, reset_code: u64) {
    ...
    B: Buf,
{
    #[cfg_attr(feature = "tracing", instrument(skip_all, level = "trace"))]
    fn poll_ready(&mut self, cx: &mut task::Context<'_>) -> Poll<Result<(), StreamErrorIncoming>> {
        if self.writing.is_none() && !self.write_in_progress {
            return Poll::Ready(Ok(()));
        }

        if let Some(mut stream) = self.stream.take() {
            if let Some(mut chunks) = self.writing.take() {
                self.write_in_progress = true;
                self.write_chunks_fut.set(async move {
                    let result = stream.write_all_chunks(&mut chunks).await;
                    (stream, result)
                });
            }
        }

        let (mut stream, result) = ready!(self.write_chunks_fut.poll(cx));
        self.write_in_progress = false;

        if let Some(reset_code) = self.pending_reset.take() {
            Self::reset_quinn_stream(&mut stream, reset_code);
        }

        self.stream = Some(stream);
        Poll::Ready(result.map_err(convert_write_error_to_stream_error))
    }
    ...
    fn reset(&mut self, reset_code: u64) {
        if let Some(stream) = self.stream.as_mut() {
            Self::reset_quinn_stream(stream, reset_code);
        } else {
            self.pending_reset = Some(reset_code);
        }
    }

    #[cfg_attr(feature = "tracing", instrument(skip_all, level = "trace"))]
    fn send_data<D: Into<WriteBuf<B>>>(&mut self, data: D) -> Result<(), StreamErrorIncoming> {
        if self.writing.is_some() || self.write_in_progress {
        ...
        self.writing = Some(data.into().into_bytes_chunks());
        Ok(())
    }
