...
- ✅ **DNS Caching** - In-memory DashMap with TTL, background refresh at 75%, startup warmup (backends + upstreams + plugin endpoints, deduplicated), static overrides, shared cache for plugin outbound calls via `GatewayDnsResolver`
