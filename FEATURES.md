...
- **Host-only routing** — HTTP proxies can match purely on `hosts` with no `listen_path`. A host-only proxy serves any path under the configured host (Kong-style). Per-host matching order: exact path → regex path → host-only fallback.
