...
- Non-success responses are reported by status without draining their bodies. Any drain bail-out (non-success status, `Content-Length` over 1 MiB, streaming abort at 1 MiB, or transport error) drops the response without consuming the body, which closes the underlying HTTP connection and forces a fresh TLS handshake on the next dispatch; this is acceptable for the typical alert cadence (up to a few notifications per second per channel) and avoids spending work on misbehaving endpoints.
