...
| `FERRUM_MESH_FEDERATION_FAIL_OPEN` | No | `false` | When `true`, a trust domain with no successfully cached bundle does not block cross-cluster mTLS — verification falls through whatever the CP-supplied slice provides (which may be empty). Default `false` means missing bundles fail closed: cross-cluster mTLS verifies only against the last-good bundle. Pollers always preserve the last-good bundle on poll failure regardless of this flag |
