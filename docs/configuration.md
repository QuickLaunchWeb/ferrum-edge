...
| `FERRUM_MESH_WORKLOAD_LABELS` | No | — | Workload labels for this mesh data plane (`k1=v1,k2=v2`). Drives `mesh_authz` and `PeerAuthentication` `PolicyScope` filtering — only policies whose scope (`MeshWide`, `Namespace`, or `WorkloadSelector`) matches these labels apply to this proxy. The Kubernetes injector (Phase D) populates this from pod labels via the downward API; set explicitly for non-K8s deployments |
