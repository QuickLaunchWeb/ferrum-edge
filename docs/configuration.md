...
| `FERRUM_NODE_AGENT_CNI_ENABLED` | No | `false` | Opts in to the CNI-style install. When `true`, the node-agent binds a Unix socket and listens for ADD/DEL/CHECK calls forwarded by the `ferrum-cni` binary that the Helm chart drops into `/opt/cni/bin/`. The kube-rs watcher remains the source of truth for enrollment; the CNI hook closes the kubelet-vs-watcher race at sandbox setup time. See [node_agent.md](node_agent.md#cni-plugin-install-optional) |
