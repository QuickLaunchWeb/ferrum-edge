...
| `FERRUM_MESH_VS_HEADER_ROUTING_EXPERIMENTAL` | No | `false` | Opt-in: emit `mesh_route_dispatch` plugin instances for Istio VirtualService routes with method/header/queryParam predicates. The K8s translator captures the predicates as plugin config (with `reject_unmatched: true` so requests that miss the predicates return 404 instead of falling through to the default backend, preserving VS match semantics); the data plane plumbs route overrides through `RequestContext.route_override_*` so downstream pool keys, capability registry, and circuit-breaker target keys derive from the effective destination. Admission plugins still evaluate the original public proxy identity. Default false (predicates dropped — existing behavior). Multi-destination canary routing is a tracked follow-up |
