...
HBONE CONNECT streams are always kept in streaming mode through authentication so the HTTP/2 upgrade handle remains available to the tunnel relay. Request-body plugins that run after authentication are skipped by the HBONE relay path, and auth plugins that normally require pre-auth body buffering (for example digest-backed `hmac_auth`) authenticate against the CONNECT headers and an empty request body digest instead of consuming DATA frames before the tunnel is established.
