...
| Variable | Required | Default | Description |
|---|---|---|---|
| `FERRUM_MESH_CONFIG_PROTOCOL` | No | `native` | Mesh config source. Mesh runtime currently supports `native` Ferrum `MeshSubscribe`; `xds` is rejected until the mesh DP xDS client is wired. `FERRUM_XDS_ENABLED` only exposes CP ADS for Envoy-compatible clients |
