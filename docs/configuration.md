...
| `FERRUM_MESH_IP6TABLES_ENABLED` | No | `auto` | IPv6 iptables fan-out: `auto` probes and skips IPv6 rules when `ip6tables` is unavailable, `true` requires it when IPv6 CIDRs are configured, `false` emits IPv4-only capture rules |
...
| `FERRUM_MESH_IP6TABLES_ENABLED` | No | `auto` | IPv6 iptables fan-out for injected init containers and capture fallback: `auto` probes and skips IPv6 rules when `ip6tables` is unavailable, `true` requires it whenever IPv6 CIDRs are configured, `false` emits IPv4-only capture rules |
