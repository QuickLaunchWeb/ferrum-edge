...
Istio `DestinationRule` resources are translated: `connectionPool.tcp.connectTimeout` maps to `backend_connect_timeout_ms`, `outlierDetection` maps to passive health checks (`consecutive5xxErrors`, `interval`, `baseEjectionTime`), `loadBalancer` maps to Ferrum algorithms (`ROUND_ROBIN`, `LEAST_REQUEST`/`LEAST_CONN`, `RANDOM`, `consistentHash`), `loadBalancer.localityLbSetting` honors `enabled`, weighted `distribute`, and region `failover` on top of the priority-tier preference projected from `source_locality`, and `subsets` are preserved with per-subset traffic policy overrides. The translator rejects combined locality-LB modes (`distribute`, `failover`, `failoverPriority`) and rejects unsupported `failoverPriority` instead of silently dropping it. DestinationRule TLS settings (`trafficPolicy.tls`) and port-level traffic policy overrides are deferred — use per-proxy `backend_tls_*` fields.
