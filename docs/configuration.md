...
| `FERRUM_CP_REQUIRE_NAMESPACE_CLAIM` | No | `false` | When `true`, the CP requires every DP/mesh JWT to carry an `ns` claim (single string or array of strings) naming the namespaces the bearer is authorised to subscribe to. Subscriptions whose advertised namespace is not in the claim are rejected with gRPC `PERMISSION_DENIED`. Multi-tenant deployments should enable this to make CP scope alone insufficient for cross-tenant access. Default `false` preserves the pre-T2-A behavior where the CP only consults its own scope. See [cp_namespace_tenancy.md](cp_namespace_tenancy.md) |
...
| `FERRUM_NODE_AGENT_FALLBACK_MODE` | No | `iptables` | Behaviour when eBPF prerequisites are missing (kernel < 5.7, cgroup v1, or bpffs unmounted). Default `iptables` keeps the node serving via host iptables capture and stamps `ferrum_mesh_node_topology_degraded=1` with a `reason` label of `kernel_too_old`/`cgroup_v1`/`bpffs_missing`. `fail` refuses startup with a structured error so DaemonSet readiness fails fast on degraded nodes. See [mesh.md](mesh.md#mixed-kernel-clusters) and [node_agent.md](node_agent.md#kernel-fallback) |
