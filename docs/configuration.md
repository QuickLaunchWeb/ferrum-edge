...
| `FERRUM_MESH_OUTBOUND_TRAFFIC_POLICY` | No | `allow_any` | Istio-compatible mesh-wide outbound policy: `allow_any` (no gate) or `registry_only` (auto-inject `mesh_outbound_registry` plugin that rejects unknown destinations with 502). Slice-supplied `outbound_traffic_policy` takes precedence when the CRD path sets it |
