...
**Single-protocol-per-request contract**: Once a request dispatches via the native H3 pool, every retry attempt for that request stays on H3. The retry loop in `proxy_to_backend_inner` captures `request_was_h3 = supports_native_http3_backend(...)` once and threads it into both the initial `proxy_to_backend` call (as `dispatch_h3`) and every retry, so `mark_h3_unsupported` firing mid-loop — or a concurrent capability refresh between the outer capture and the inner async DNS resolve — only routes the NEXT request via reqwest. The in-flight request finishes its retry attempts on H3 and the 502 propagates if every attempt fails. Same-protocol H3 retries do replay the retained body (the outer retry loop's job, gated by `proxy.retry.retry_on_methods`); what the gateway forbids is *cross-protocol* replay on the same request — the failed H3 attempt may already have flushed headers / body to the backend before the reset / timeout / protocol error surfaced, so replaying the same body via reqwest would bypass `retry_on_methods` and could duplicate non-idempotent requests. Subsequent requests then see `h3 = Unsupported` and route via the cross-protocol bridge.
