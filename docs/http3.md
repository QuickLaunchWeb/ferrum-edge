...
**Typed classifier**: `classify_h3_error` in `src/proxy/mod.rs` is a thin wrapper over `http3::client::classify_http3_error` — both walk `std::error::Error::source()` for typed `quinn::ConnectionError` / `quinn::ConnectError` / `io::Error` first, falling back to anchored substring matches on `h3::Error` Display strings only when the typed chain doesn't expose a recognizable cause. The proxy wrapper adds `(error_kind, is_connection_error)` for the BackendResponse path: `is_connection_error` gates retry-on-connect (true for connection-establishment failures, false for protocol-level / read-timeout errors that won't recover by reconnecting). The buffered consumer in `proxy_to_backend_http3` calls `drain_h3_response_body`, which returns `Result<Vec<u8>, h3::error::StreamError>` so the typed `StreamError` survives all the way to the classifier — recoveries (`H3_NO_ERROR` / `GOAWAY` after a complete body) are caught at the `recv_data` boundary inside the drain helper itself and never produce an error in the first place.
