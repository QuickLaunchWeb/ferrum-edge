...
Direct HTTP/2 responses have a narrow known-size bypass to avoid a copy when coalescing cannot help:
...
- The body must either fit within one H2 coalesce target (`FERRUM_H2_COALESCE_TARGET_BYTES`) or be at least 512 KiB. Small known responses would otherwise pay a `BytesMut` staging copy for a single frame, while large direct-H2 backend frames are already large enough that coalescing adds a copy before the existing large-frame fast path returns the original frame.

Unknown-size responses keep the coalescer because it is also the streaming size-enforcement path when `Content-Length` is absent. Mid-sized known responses above the coalesce target but below 512 KiB also keep the coalescer because they may arrive as multiple smaller backend DATA frames where write amortization still helps.

gRPC streaming responses intentionally stay on `CoalescingH2Body`; preserving trailers and large-payload batching matters more there, and local small-response bypass tests did not show a throughput win.
