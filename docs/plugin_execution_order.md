...
Per-rule `fault` actions (Istio `VirtualService.http[].fault`) also ride on `mesh_route_dispatch` rules: on match, the dispatch plugin rolls the configured percentile and, on hit, sleeps for `delay.duration_ms` then short-circuits with the configured HTTP status (or `application/grpc` trailers carrying `grpc-status` for gRPC requests). The action runs immediately after the route override is published, so no backend dispatch happens for aborted requests and downstream `before_proxy` plugins do not see them. Static `percentage` baked into the rule is not RTDS-tunable — use a separate `fault_injection` plugin with `runtime_overlay_scope` if runtime tunability matters.
