...
| `trafficPolicy.portLevelSettings[].loadBalancer` / `outlierDetection` | Supported | Per-port load-balancer algorithm/hash settings and passive outlier thresholds land on `Upstream.port_overrides[port]`; the runtime builds isolated per-port LB counters/hash rings and consults per-port passive health before the upstream default. Migration note: operators who previously set these fields expecting warning-only behavior should audit them before upgrade because they now affect routing and ejection decisions. |
