...
`distribute` and `failover` are mutually exclusive at evaluation time: a matching `distribute` entry takes precedence over `failover` (Istio behavior). The K8s translator validates each entry at admission — invalid locality strings, `from == to` self-failovers, and empty `to` maps return a translator error rather than silently dropping the policy.
