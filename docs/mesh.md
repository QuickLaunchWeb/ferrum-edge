...
- HTTP-family materialized proxies use host-only routing (no `listen_path`), `preserve_host_header: true`, and passive health checks. Stream-family proxies use port routing (no `hosts`), `passthrough: false` (the gateway terminates inbound sidecar mTLS at 15090 and re-emits plain TCP to the external backend — this is NOT raw SNI passthrough, that flow lives in the east-west gateway), and passive health checks.
