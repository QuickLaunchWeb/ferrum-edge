...
Port-list annotations merge with their Ferrum aliases and the applicable injector-level defaults; results are deduplicated. `includeOutboundPorts` narrows outbound REDIRECT rules to the listed TCP destination ports while preserving any IPv4 include CIDRs that survive plan rendering, and outbound exclude ports still win because their RETURN rules are emitted first. CIDR annotations are validated at admission time -- invalid ports or CIDRs are rejected with a webhook error that names the offending annotation, so a typo cannot silently produce a broken iptables plan.
