...
This priority selection applies before the configured Ferrum algorithm for the chosen tier, including per-port and subset selectors. DestinationRule `localityLbSetting.distribute` weights and `failover` ordering are still deferred to the weighted locality sub-PR.
...
| `WorkloadEntry` `weight` / `locality` / `serviceAccount` | Partial | `weight` and `locality` are consumed by upstream target materialization; locality priority load balancing prefers exact, zone, then region tiers before falling back. DestinationRule `localityLbSetting.distribute` / `failover` remains deferred. `serviceAccount` is kept separately from the SPIFFE path so introspection/audit doesn't need to parse it. |
