...
- `distribute[].from` matches the source workload's locality exactly or via Istio terminal wildcards such as `region/zone/*`. When a match is found the load balancer overrides the priority preference with weighted random selection: each target's effective weight is the sum of every `to[locality]` entry whose locality applies (region-only `to` entries apply to every target in that region, `region/zone` entries apply to every target in that zone, full `region/zone/subzone` entries require an exact match, and terminal wildcards such as `region/zone/*` match the covered tier). Targets that receive zero weight are excluded, and an entry that names no reachable target falls through to the rest of the locality LB path so the upstream still serves.
