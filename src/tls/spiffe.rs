...
fn verify_peer_against_bundle(
    ...
    // Use a synthetic ServerName — the SPIFFE URI is the peer identity, not
    // a DNS name. webpki still needs *something* DNS-shaped to satisfy the
    // signature path; we feed it a placeholder and rely on the SPIFFE URI
    // SAN extraction above as the actual identity check.
    let placeholder = ServerName::try_from("spiffe.invalid")
        .map_err(|e| format!("placeholder ServerName: {e}"))?;
