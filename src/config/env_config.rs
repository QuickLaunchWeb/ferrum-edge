...
impl EnvConfig {
    ...
    pub fn from_env_with_conf(conf: &ConfFile) -> Result<Self, String> {
        ...
        let config = Self {
            ...
            http3_coalesce_max_bytes: resolve_usize(
                conf,
            ...
            http3_coalesce_min_bytes: {
                // MIN must never exceed the runtime-configured MAX — clamp down
                // and warn when an operator provides MIN > MAX.
                let max = resolve_usize(
                    conf,
                    "FERRUM_HTTP3_COALESCE_MAX_BYTES",
                    crate::http3::config::H3_COALESCE_MAX_DEFAULT,
                )
                ...
                let raw = resolve_usize(
                    conf,
                    "FERRUM_HTTP3_COALESCE_MIN_BYTES",
                    crate::http3::config::H3_COALESCE_MAX_DEFAULT,
                );
                let clamped = raw.clamp(crate::http3::config::H3_COALESCE_MIN_FLOOR, max);
                if raw > max {
                    tracing::warn!(
            ...
            http3_flush_interval_micros: resolve_u64(
                conf,
                "FERRUM_HTTP3_FLUSH_INTERVAL_MICROS",
                200,
            )
            .clamp(
                crate::http3::config::H3_FLUSH_INTERVAL_MIN_MICROS,
                crate::http3::config::H3_FLUSH_INTERVAL_MAX_MICROS,
            ),
            http3_initial_mtu: resolve_u16(conf, "FERRUM_HTTP3_INITIAL_MTU", 1500),
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        // HTTP/3 initial MTU must be in quinn's accepted range. Out-of-range
        // values are rejected rather than silently clamped — a 900-byte MTU or
        // 70000-byte MTU reflects a real misconfiguration worth surfacing.
        ...
        {
            return Err(format!(
                "Invalid FERRUM_HTTP3_INITIAL_MTU '{}'. Must be in [{}, {}] (quinn's accepted range)",
