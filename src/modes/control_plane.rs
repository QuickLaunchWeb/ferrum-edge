                    ...
                    if let Some(since) = last_poll_at {
                        ...
                        match db_poll.load_incremental_config(
                            ...
                            Ok(result) => {
                                ...
                                // Re-derive the delta from the validated config for
                                // broadcast. The result was consumed by
                                // apply_incremental_to_config, but the broadcast only
                                // needs the new_config snapshot — use a full broadcast
                                // since the validated config is the authoritative state.
                                CpGrpcServer::broadcast_update_with_registry(&update_tx, &new_config, &dp_registry_poll);

                                config_poll.store(Arc::new(new_config));

                                info!(
                                    "Incremental config update validated and pushed to DPs (version={})",
                                    version
                                );
