...
- `FERRUM_MESH_CAPTURE_MODE` (`explicit`; `iptables` for injector init-container capture, `ebpf` for eBPF with runtime kernel-version check falling back to iptables on kernels below 5.7); `FERRUM_MESH_PROXY_UID` (1337 in injected sidecars); `FERRUM_MESH_EGRESS_STRIP_BAGGAGE_KEYS` (comma-separated key prefixes stripped at egress dispatch)
