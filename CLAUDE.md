...
**Mesh data model** (`config.rs`): `Workload` (SPIFFE identity unit), `MeshService` (logical service grouping workloads), `MeshPolicy` (authorization with PolicyScope + rules), `PeerAuthentication` (per-port mTLS mode), `ServiceEntry` (external service hosts/endpoints/resolution/location/export_to), `MeshRequestAuthentication` (JWT validation rules with scope — permissive: declares valid JWTs, not required ones), `MeshTelemetryResource` (per-scope tracing/metrics/access-logging config), `TrustBundleSet` (local + federated X.509/JWT authorities), `MultiClusterConfig` (remote clusters + east-west gateways), `MeshProxyConfig` (Istio `ProxyConfig` — PolicyScope-scoped (root-namespace → MeshWide), `tracing_sampling` merges into `workload_metrics.sampling_percentage`, invalid `spec.concurrency` rejected as InvalidResource). Validation: `validate_mesh_config()` returns flat `Vec<String>` errors.
