...
|------|---------|---------------|----------|-----------|
| **Fast path** | `FERRUM_TCP_IDLE_TIMEOUT_SECONDS=0` **AND** `FERRUM_TCP_HALF_CLOSE_MAX_WAIT_SECONDS=0` **AND** per-proxy `backend_read_timeout_ms=0` **AND** per-proxy `backend_write_timeout_ms=0` (all four must be zero) | Delegates to `tokio::io::copy_bidirectional_with_sizes` directly. No `tokio::io::split`. No Phase 1/Phase 2 select loop. Per-direction buffers are managed internally by tokio. | Zero BiLock overhead on reads/writes. No select-loop scheduling. Per-direction byte counts are preserved on clean completion. Best raw TCP throughput. | **No idle watchdog** — stalled sessions persist until the OS TCP keep-alive/timer fires (Linux default ~2 h via `net.ipv4.tcp_keepalive_*`). **No half-close hard cap** — one-sided-close + stalled peer holds the task open. **`disconnect_direction` is always `unknown` on error** because `copy_bidirectional_with_sizes` does not expose which half failed first. `disconnect_cause` is never `IdleTimeout`. |
| **Direction-tracking path** (default) | Any of the four timeouts non-zero | `tokio::io::split` on both sides, two `copy_one_direction` futures, Phase 1 race for first-completion + Phase 2 drain for the remaining half. `last_activity` is refreshed on every read for idle detection. Per-direction `backend_read_timeout_ms` / `backend_write_timeout_ms` wrap the individual `read`/`write` calls in `tokio::time::timeout` so a silent or back-pressured backend trips `ReadWriteTimeout` even when both sides of the bidirectional `idle_timeout` watchdog are still nominally "active". | Idle timeout enforcement. Hard cap enforcement on half-close drain. Per-direction read/write timeout enforcement (HAProxy `timeout server` / nginx `proxy_read_timeout` semantics). Per-direction byte counters with first-failure direction attribution (`disconnect_direction` populated in `StreamTransactionSummary` / Prometheus `stream_disconnects`). | One BiLock access per read and per write (~5 ns each). Two `Vec<u8>` buffer allocations per connection (4–64 KB each, one per direction). Phase 1/Phase 2 scheduling adds a few extra `tokio::select!` wakeups per connection. |
...
**Semantic opt-in**: this is controlled via the existing timeouts rather than a dedicated perf flag because the architectural requirements are entangled — the idle watchdog needs `tokio::io::split` to refresh `last_activity` on each read, the half-close cap needs the Phase 1/Phase 2 loop, and the per-direction `backend_read_timeout_ms` / `backend_write_timeout_ms` need `copy_one_direction` to wrap the individual `read`/`write` futures in `tokio::time::timeout`. A separate `FERRUM_TCP_DIRECTION_TRACKING=false` variable would either be redundant (equivalent to zeroing all four knobs) or silently no-op (ignored when any is set).

**`backend_read_timeout_ms` / `backend_write_timeout_ms` defaults are HTTP-shaped (30 s)**: these fields are shared with the HTTP/gRPC backend pool path and default to 30,000 ms. With this PR they also bind on the raw TCP relay (b2c read for `backend_read_timeout_ms`, c2b write for `backend_write_timeout_ms`). For long-lived TCP workloads (database keep-alives, message-broker streams, SSH/IMAP passthrough) the 30 s default will close otherwise-healthy connections — set both to `0` on the proxy to disable, falling back to `tcp_idle_timeout_seconds` + the OS TCP keep-alive. Splice and kTLS-splice paths skip these timeouts (no per-read hook in the kernel relay) — they rely on `tcp_idle_timeout_seconds` exclusively. The bench harness sets `FERRUM_TCP_IDLE_TIMEOUT_SECONDS=30` and `FERRUM_TCP_HALF_CLOSE_MAX_WAIT_SECONDS=30` for synthetic scenarios; production deployments should pick TCP-appropriate values.
