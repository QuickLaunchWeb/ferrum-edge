...
**Single-protocol-per-request contract**: Within a single request, retries stay on the protocol the request started with — H3 retries hit the H3 pool, reqwest retries hit reqwest. Same-protocol H3 retries do replay the retained body (gated by `proxy.retry.retry_on_methods`); what's forbidden is *cross-protocol* replay, which would bypass that policy and risk duplicating non-idempotent requests since the failed transport attempt may have flushed headers/body before the error surfaced. The retry loop in `proxy_to_backend_inner` captures `request_was_h3 = supports_native_http3_backend(...)` once and threads it into both the initial attempt (as `dispatch_h3` to `proxy_to_backend`) and every retry — re-reading the registry inside `proxy_to_backend` would race the async DNS resolve against a concurrent `mark_h3_unsupported` / refresh and split the first attempt and the retries across transports. Capability downgrade affects the NEXT request, never this one. This matches industry practice (Envoy, NGINX, HAProxy, Cloudflare, GFE): cross-protocol fallback happens at connection establishment (happy-eyeballs), not mid-request.
